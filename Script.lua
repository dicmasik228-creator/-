...
-- ============================================
-- Анти Граб (Объединённая версия, без кика)
-- ============================================
...
local function disableBlobmanWeld(blobman)
    ...
    if leftDetector then
        ...
        local leftAlign = leftDetector:FindFirstChild("LeftAlignOrientation")
        if leftAlign then leftAlign.Enabled = false end
    end
    ...
    if rightDetector then
        ...
        local rightAlign = rightDetector:FindFirstChild("RightAlignOrientation")
        if rightAlign then rightAlign.Enabled = false end
    end
end

local function startAntiBlobman()
    if antiBlobmanLoop then antiBlobmanLoop:Disconnect() end
    
    for _, blobman in pairs(workspace:GetDescendants()) do
        if blobman.Name == "CreatureBlobman" then
            local folderName = LocalPlayer.Name .. "SpawnedInToys"
            if blobman.Parent and blobman.Parent.Name ~= folderName then
                disableBlobmanWeld(blobman)
            end
        end
    end
    
    antiBlobmanLoop = workspace.DescendantAdded:Connect(function(descendant)
        if not antiBlobmanActive then return end
        if descendant.Name == "CreatureBlobman" then
            task.wait(0.1)
            local folderName = LocalPlayer.Name .. "SpawnedInToys"
            if descendant.Parent and descendant.Parent.Name ~= folderName then
                disableBlobmanWeld(descendant)
            end
        end
    end)
end
...
local function stopAntiBlobman()
    if antiBlobmanLoop then
        antiBlobmanLoop:Disconnect()
        antiBlobmanLoop = nil
    end
end

DefenseRightGroup:AddToggle("AntiBlobman", {
    Text = "Анти Блобмен",
    Default = false,
    Callback = function(Value)
